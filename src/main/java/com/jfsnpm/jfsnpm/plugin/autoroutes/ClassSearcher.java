			...
			for (int i = 0; i < files.length; i++) {
				...
				if (file.isDirectory()) {
					...
					if (wildcardMatch(targetFileName, file.getName())) {
						...
						int end = fileName
								.indexOf(close, start + open.length());
						String className = fileName.substring(
								start + open.length(), end).replaceAll(
		...
		if (scanPackages.isEmpty()) {
			...
			for (String scanPackage : scanPackages) {
				classFileList = findFiles(classpath + File.separator
						+ scanPackage.replaceAll("\\.", "\\" + File.separator),
						"*.class");
			}
		}
			...
			for (File file : files) {
				if (file.isDirectory()) {
					...
					if (includeAllJarsInLib
							|| includeJars.contains(file.getName())) {
						JarFile localJarFile = null;
						try {
							localJarFile = new JarFile(new File(baseDirName
									+ File.separator + file.getName()));
							Enumeration<JarEntry> entries = localJarFile
									.entries();
