...
public class UserRemoteImpl4demo implements IUserRemote {
	public User checkToken(String token,Controller c) {
		...
		if(user != null){
			...
			//add for demo 添加用户默认添加到admin角色
			Record roleAdd = Db.findFirst("SELECT * FROM jfsnpmr_role_user WHERE "
					+ "roleId = 'admin' AND userId = ? ", userRecord.getStr("id"));
			if(roleAdd == null){
				Db.save("jfsnpmr_role_user", new Record().set("roleId", "admin").
						set("userId", userRecord.getStr("id")));
			}
			//add-end
			return user;
		}
